...
public class TopLuckManager {
    ...
    public static void reloadTopLuck() {
        ...
        for (String matId : Main.getInstance().getConfig().getStringList("materialsverified")) {
            String typeName = matId.split(":")[0].split(";")[0].toUpperCase();
            if (Material.getMaterial(typeName) != null) {
                String displayItemType = typeName;
                try {
                    displayItemType = matId.split(";")[1];
                } catch (ArrayIndexOutOfBoundsException ignore) {}
                System.out.println(displayItemType);
                MaterialType type = new MaterialType(typeName, displayItemType);
    ...
    public static void resetPlayer(Player player) {
        ...
        for (MaterialType type : blockCounted) {
            topLuck.set(player.getName() + "." + type.getType().name(), 0);
        }

        saveTopLuck();
    }

    public static void mine(Player player, String materialName) {
        if (!knowPlayer(player)) {
            resetPlayer(player);
        }
        topLuck.set(player.getName() + "." + materialName, topLuck.getInt(player.getName() + "." + materialName) + 1);
        saveTopLuck();

        Map<String, Integer> map = blockBreakedFromLastConnexion.get(player.getName());
        map.put(materialName, map.getOrDefault(materialName, 0) + 1);
    }
    ...
    public static boolean containsBlock(MaterialType materialType) {
        for (MaterialType type : blockCounted) {
            if (materialType.getType() == type.getType()) {
                return true;
            }
        }
        return false;
    ...
    public static int getResourceMinedFromLastCo(Player player, MaterialType type) {
        if (type.getType() == Material.STONE) {
            return blockBreakedFromLastConnexion.get(player.getName()).getOrDefault("stone", 0);
        }
        return blockBreakedFromLastConnexion.get(player.getName()).getOrDefault(type.getType().name(), 0);
    }
    ...
    public static int getShowedStoneMined(Player player) {
        if (Main.getInstance().getConfig().getBoolean("showlastonprofil")) {
            return getResourceMinedFromLastCo(player, new MaterialType(Material.STONE.name()));
        }
    ...
    public static String getLastStonePercent(Player player) {
        ...
        double total = getResourceMinedFromLastCo(player, new MaterialType(Material.STONE.name()));
        for (MaterialType type : blockCounted) {
            total += getResourceMinedFromLastCo(player, type);
        }
        return (getResourceMinedFromLastCo(player, new MaterialType(Material.STONE.name())) / total * 100 > 0) ? result.format(getResourceMinedFromLastCo(player, new MaterialType(Material.STONE.name())) / total * 100) : "0.00";
    }
    ...
    public static String getTotalStonePercent(Player player) {
        ...
        for (MaterialType type : blockCounted) {
            total += topLuck.getInt(player.getName() + "." + type.getType().name());
        }
    ...
    public static int getTotalOresMined(Player player, MaterialType type) {
        return topLuck.getInt(player.getName() + "." + type.getType().name());
    }
    ...
    public static String getTotalOrePercent(Player player, MaterialType materialType) {
        ...
        return (topLuck.getInt(player.getName() + "." + materialType.getType().name()) / total * 100 > 0) ? result.format(topLuck.getInt(player.getName() + "." + materialType.getType().name()) / total * 100).replace(",", ".") : "0.00";
    }

    public static void addResource(ItemStack item, String displayItemType) {
        if (!containsBlock(new MaterialType(item.getType().name(), displayItemType))) {
            blockCounted.add(new MaterialType(item.getType().name(), displayItemType));
            List<String> blocks = Main.getInstance().getConfig().getStringList("materialsverified");
            blocks.add(item.getType().name() + ";" + displayItemType);
