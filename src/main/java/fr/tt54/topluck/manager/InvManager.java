...
public class InvManager {
    ...
    public static Inventory getTopLuckInventory(int page) {
        ...
        for (int i = page * 9 * 5; i < (Math.min(players.size(), (page + 1) * 9 * 5)); i++) {
            ItemBuilder builder = new ItemBuilder(new ItemStack(Material.PLAYER_HEAD, 1)).setName("§6" + (players.get(i)).getName()).setSkullOf(players.get(i))
    ...
    public static Inventory getTopLuckPlayerInventory(String playerName) {
        ...
        stoneBuilder.addLoreLine(Main.getMessages().getMessage("inventory.lastconnection", "%percent%", TopLuckManager.getLastStonePercent(Bukkit.getPlayer(playerName)), "%number%", "" + TopLuckManager.getResourceMinedFromLastCo(Bukkit.getPlayer(playerName), new MaterialType(Material.STONE.name()))));
        ...
        for (int i = 0; i < Math.min(9 * 5 - 1, TopLuckManager.blockCounted.size()); i++) {
            ...
            ItemBuilder builder = new ItemBuilder(new ItemStack(Material.getMaterial(type.getDisplayName()), 1));
